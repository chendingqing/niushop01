...
<div class="set-style">
	<dl>
		...
		<dd>
			<input id="title" type="text" value="<?php echo $document_detail['title']; ?>" class="input-common long" />
		</dd>
	</dl>
		...
		<dd>
			<div class="upload-btn-common">
				<div>
					...
					<input type="hidden" id="logo" value="<?php echo $document_detail['image']; ?>">
				</div>
				
				<input type="text" id="text_logo" class="input-common" readonly="readonly" value="<?php echo $document_detail['image']; ?>">
				<em>上传</em>								
				<img id="preview_logo" src="__STATIC__/blue/img/upload-common-select.png" data-src="<?php echo __IMG($document_detail['image']); ?>" data-html="true" data-container="body" data-placement="top" data-trigger="manual" data-original-title="" title="">
			</div>
			<p class="hint">
	...
	<dl style="display:none;">
		<dt>链接地址：</dt>
		<dd>
			<input id="link_url" type="text" value="<?php echo $document_detail['link_url']; ?>" class="input-common long" />
		</dd>
	</dl>
	<dl>
		<dt>所属分类：</dt>
	...
	<dl>
		<dt>是否显示：</dt>
		<dd>
			<input id="is_visibility" type="checkbox" class="checkbox" <?php if($document_detail['is_visibility']): ?>checked="checked"<?php endif; ?>/>
		</dd>
	</dl>
	<dl>
		<dt>排序：</dt>
		<dd>
			<input id="sort" type="number" value="<?php echo $document_detail['sort']; ?>" onkeyup="this.value=this.value.replace(/\D/g,'')" class="input-common harf" />
		</dd>
	</dl>
	...
	$.ajax({
		...
		success : function(data) {
			if (data["code"] > 0) {
				...
				window.location.href = "<?php echo __URL('ADMIN_MAIN/config/helpdocument'); ?>";
			} else {
