...
<div class="set-style">
	...
	<dl>
		<dt>是否显示：</dt>
		<dd>
			<input id="is_visibility" type="checkbox" class="checkbox" checked="checked"/>
		</dd>
	</dl>
	<dl>
		<dt>排序：</dt>
		<dd>
			<input id="sort" class="input-common harf" type="number" onkeyup="this.value=this.value.replace(/\D/g,'')" />
		</dd>
	</dl>
...
function setConfigAjax() {
	...
	if(!if_submit){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('ADMIN_MAIN/config/adddocument'); ?>",
			data : {
				'title' : title,
				'class_id' : class_id,
				'link_url' : link_url,
